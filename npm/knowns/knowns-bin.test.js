...
function createPlatformPackage() {
  ...
  fs.writeFileSync(path.join(packageDir, "knowme.exe"), "binary");
  fs.writeFileSync(path.join(packageDir, "onnxruntime.dll"), "dll");
  return { binary: path.join(packageDir, "knowme.exe"), cacheRoot };
}
...
test("stages the Windows binary and adjacent DLLs outside node_modules", () => {
  ...
  expect(staged).toBe(path.join(cacheRoot, "1.2.3-x64", "knowme.exe"));
