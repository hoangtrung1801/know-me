...
func renderOpenCodePlugin(opts Options) string {
	...
	return strings.Join([]string{
		...
		"  const logPath = join(process.env.HOME || process.cwd(), \".know-me\", \"runtime\", \"opencode-runtime-memory.log\")",
		"  const log = (message, extra) => {",
		"    if (!debugEnabled) return",
		"    try {",
		"      mkdirSync(join(process.env.HOME || process.cwd(), \".know-me\", \"runtime\"), { recursive: true })",
