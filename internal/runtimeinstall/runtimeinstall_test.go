...
func TestInstallClaudeWindowsQuotesExecutableForBashHooks(t *testing.T) {
	...
	exePath := `C:\Users\Admin\.know-me\bin\knowme.exe`
	opts := Options{
		HomeDir:        home,
...
func TestInstallCodexNormalizesDeprecatedFeatureAndUninstallRemovesManagedHookOnly(t *testing.T) {
  ...
  "hooks": {
    "SessionStart": [
      ...
      {"hooks": [{"type": "command", "command": "/tmp/old-knowme runtime-memory hook --runtime codex --event session-start", "statusMessage": "Knowns runtime memory"}]}
    ]
  }
}`
