...
Recommended context flow:
  1. Use 'knowme code search <query>' for keyword code discovery across LSP symbols.
  2. Use 'knowme code symbols' to verify what was actually indexed in a file or scope.
  3. Use 'knowme code deps' to inspect raw relationships such as calls, imports, ownership, and inheritance.

Examples:
  knowme code search "login auth"
  knowme code search "handleCodeDefinition" --path internal/mcp
  knowme code deps --type calls
  knowme code symbols --kind function`,
}
...
func runCodeDeps(cmd *cobra.Command, args []string) error {
	...
	if db == nil {
		if isJSON(cmd) {
			...
			fmt.Println(RenderWarning("No code dependency index found. Run 'knowme code ingest' first."))
		}
		return nil
	}
	defer db.Close()
...
func runCodeSymbols(cmd *cobra.Command, args []string) error {
	...
	if db == nil {
		if isJSON(cmd) {
			...
			fmt.Println(RenderWarning("No code symbol index found. Run 'knowme code ingest' first."))
		}
		return nil
	}
	defer db.Close()
