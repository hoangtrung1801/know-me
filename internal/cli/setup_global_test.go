...
func TestSetupGlobalClaudeCodeMCPUsesClaudeUserConfig(t *testing.T) {
	...
	if got := knowns["command"]; got != "knowme" {
		t.Fatalf("expected knowme MCP command, got %#v", got)
	}
	args, ok := knowns["args"].([]any)
	if !ok {
		t.Fatalf("expected knowme MCP args to be []any, got %T", knowns["args"])
	}
	wantArgs := []string{"mcp", "--stdio"}
	if len(args) != len(wantArgs) {
		t.Fatalf("expected %d knowme MCP args, got %d", len(wantArgs), len(args))
	}
	for i, want := range wantArgs {
		if args[i] != want {
			t.Fatalf("expected knowme MCP arg %d to be %q, got %#v", i, want, args[i])
		}
	}
