...
func runStatusCheck() error {
	...
	if !usesLocalONNX {
		...
		fmt.Println(searchDimStyle.Render("    Reinstall knowme to restore the ONNX runtime library"))
	}
	...
	if cfg != nil && cfg.Settings.SemanticSearch != nil {
		...
		fmt.Println(searchDimStyle.Render("    Set up: knowme search --setup"))
	}
	...
	if count > 0 {
		...
		fmt.Println(searchDimStyle.Render("    Build: knowme search --reindex"))
	}
	...
	if providerReady && semanticSettings != nil && semanticSettings.Enabled && count > 0 {
		...
		fmt.Println(searchWarnStyle.Render("  Status: needs search reindex (run: knowme search --reindex)"))
	} else {
...
func runSetup() error {
	...
	if provider == "api" || provider == "ollama" {
		...
		fmt.Println(RenderNextSteps(
			...
			RenderCmd("knowme search \"your query\""),
		))
		return nil
	}
	...
	if !onnxAvail {
		fmt.Println(searchWarnStyle.Render("ONNX runtime not found."))
		fmt.Println()
		fmt.Println(RenderHint("Reinstall knowme to restore the ONNX runtime library."))
		fmt.Println()
		return nil
	}
