...
func TestLoadPluginAdaptersParsesManifest(t *testing.T) {
	...
	writePluginManifest(t, dir, "toy.json", `{
		...
		"install_guide": {"command": "npm install -g toy-lsp", "knowns_cmd": "knowme lsp install toy", "notes": "Requires Node"},
...
func TestPluginAdapterRuntimeStatusMetadata(t *testing.T) {
	...
	writePluginManifest(t, dir, "toy.json", `{
		...
		"install_guide":{"knowns_cmd":"knowme lsp install toy"}
	}`)
	...
	if status.InstallCmd != "knowme lsp install toy" {
