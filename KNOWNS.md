...
- Prefer Know-Me MCP tools; use the `knowme` CLI only as fallback.
...
- Use MCP `retrieve` tool when a workflow needs structured context with citations and context-pack assembly. Fall back to CLI `knowme retrieve` if MCP is unavailable.
