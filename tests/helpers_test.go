...
// getBinaryPath returns the absolute path to the knowme binary.
// Respects TEST_BINARY env var; defaults to ../bin/knowme relative to this source file.
func getBinaryPath(t *testing.T) string {
	...
	// so ../bin/knowme should work, but let's make it absolute.
	binaryName := "knowme"
...
func ensureTestBinary(t *testing.T, abs string) {
	...
	ensureBinaryOnce.Do(func() {
		...
		cmd := exec.Command("go", "build", "-o", abs, "./cmd/knowme")
		cmd.Dir = repoRoot
...
// setupTestProject creates an isolated temp directory with git init + knowme init.
...
func setupTestProject(t *testing.T) string {
	...
	if res.ExitCode != 0 {
		t.Fatalf("knowme init failed (code %d): %s\n%s", res.ExitCode, res.Stderr, res.Stdout)
	}

	return dir
}

// runCli executes the knowme binary with the given args inside dir.
...
// startMCPServer spawns "knowme mcp" and returns a client.
