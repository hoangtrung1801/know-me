...
type SSEEventCallback<T extends SSEEventType> = (
...
function parseTaskDTO(dto: Record<string, unknown>): Task {
    return {
        ...
        acceptanceCriteria:
            (dto.acceptanceCriteria as Task["acceptanceCriteria"]) || [],
        ...
        archivedAt: dto.archivedAt
            ...
            : undefined,
        archived:
            (dto.archived as boolean) ?? dto.lifecycleState === "archived",
        lifecycleState: dto.lifecycleState as Task["lifecycleState"],
        timeEntries: (
            (dto.timeEntries as Array<Record<string, unknown>>) || []
        ).map((entry) => ({
            ...entry,
...
export function SSEProvider({ children }: { children: ReactNode }) {
    ...
    const listenersRef = useRef<
        Map<SSEEventType, Set<SSEEventCallback<SSEEventType>>>
    >(new Map());
    ...
    const emit = useCallback(
        <T extends SSEEventType>(event: T, data: SSEEventPayloads[T]) => {
            const listeners = listenersRef.current.get(event);
            if (listeners) {
                for (const callback of listeners) {
                    try {
                        ...
                        console.error(
                            `Error in SSE event listener for ${event}:`,
                            error,
                        );
                    }
                }
    ...
    useEffect(() => {
        ...
        const addHandler = (
            ...
            const loggedHandler = (e: MessageEvent) => {
                console.log(`[SSE] ${event}`, e.data);
                handler(e);
            };
            eventSource.addEventListener(event, loggedHandler);
            handlers.push({ event, handler: loggedHandler });
        };
        ...
        const connect = () => {
            ...
            addHandler(eventSource, "tasks:lifecycle", (e) => {
                const data = JSON.parse(e.data) as Omit<
                    ...
                    "at"
                > & { at: string };
                if (!data.id || seenLifecycleEventIDsRef.current.has(data.id))
                    return;
                seenLifecycleEventIDsRef.current.add(data.id);
                if (seenLifecycleEventIDsRef.current.size > 512) {
                    const oldest = seenLifecycleEventIDsRef.current
                        .values()
                        .next().value;
                    if (oldest) seenLifecycleEventIDsRef.current.delete(oldest);
                }
                emit("tasks:lifecycle", {
                    event: { ...data, at: new Date(data.at) },
                });
                emit("tasks:refresh", {});
            });
