...
export default function MemosPage() {
	...
	return (
		<PageShell>
			...
			/>
			<PageContent size="wide" data-document-surface="memos">
				<div className="grid gap-4 lg:grid-cols-[minmax(0,1.35fr)_minmax(18rem,0.65fr)]">
					<form onSubmit={add} className="rounded-xl border border-border bg-card p-5 shadow-sm">
					...
					<div className="rounded-xl border border-border bg-muted/30 p-5">
						...
						{availableTags.length > 0 && <div className="mt-4"><div className="mb-2 flex items-center gap-2 text-xs font-medium text-muted-foreground"><Hash className="h-3.5 w-3.5" />Tags</div><div className="flex flex-wrap gap-2" aria-label="Filter memos by tag"><Button size="sm" variant={selectedTags.length === 0 ? "default" : "outline"} onClick={() => setSelectedTags([])}>All tags</Button>{availableTags.map((tag) => <Button key={tag} size="sm" variant={selectedTags.includes(tag) ? "default" : "outline"} aria-pressed={selectedTags.includes(tag)} onClick={() => setSelectedTags((current) => current.includes(tag) ? current.filter((item) => item !== tag) : [...current, tag])}>{tag}</Button>)}</div></div>}
					</div>
				</div>
				...
				{loading ? <PageLoading label="Loading memos" /> : groups.length === 0 ? <div className="mt-8 rounded-xl border border-dashed px-6 py-12 text-center text-sm text-muted-foreground">{emptyMessage}</div> : <div className="mt-8 space-y-8">
					{groups.map((group) => <section key={group.label} aria-labelledby={`memos-${group.label}`}>
						...
						<div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
							{group.items.map((memo) => <article key={memo.id} className="group flex min-h-52 flex-col rounded-xl border border-border bg-card p-5 shadow-sm transition-colors hover:border-foreground/20 hover:bg-accent/20">
								{editingID === memo.id ? <div>
									...
									<div className="mt-auto flex items-center justify-between gap-3 border-t pt-4 text-xs text-muted-foreground"><time dateTime={memo.updatedAt}>{new Date(memo.updatedAt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}</time><div className="flex gap-1 opacity-70 transition-opacity group-hover:opacity-100"><Button type="button" variant="ghost" size="icon" className="h-8 w-8" aria-label="Edit memo" onClick={() => { setEditingID(memo.id); setEditContent(memo.content); }}><Pencil className="h-4 w-4" /></Button><Button type="button" variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" aria-label="Delete memo" onClick={() => setDeleteTarget(memo)}><Trash2 className="h-4 w-4" /></Button></div></div>
								</>}
							</article>)}
						</div>
					</section>)}
