...
## What you will see during `knowme init`
...
After init, run `knowme setup <target> --global` to configure user-level AI platform integrations such as skills, MCP configs, and runtime hooks. This is the recommended setup for personal assistant usage across repositories. Use `knowme setup <target>` only when you intentionally want repo-local integration files, or `knowme setup agents` if you only need lightweight repo-local shims such as `AGENTS.md`.
...
4. Use `knowme search` or `knowme retrieve` to confirm the context can be found.
